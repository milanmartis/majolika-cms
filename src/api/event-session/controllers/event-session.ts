...
export default factories.createCoreController('api::event-session.event-session', ({ strapi }) => ({
  ...
  async listForDay(ctx) {
    ...
    if (typeof date !== 'string') return ctx.badRequest('Invalid date');
  
    // ✅ date očakávame ako "YYYY-MM-DD" a berieme ho ako UTC deň
    const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(date);
    if (!m) return ctx.badRequest('Invalid date format, expected YYYY-MM-DD');
  
    const y = Number(m[1]);
    const mo = Number(m[2]) - 1; // 0-based
    const d = Number(m[3]);
  
    const dayStartUtc = new Date(Date.UTC(y, mo, d, 0, 0, 0, 0));
    const dayEndUtc = new Date(Date.UTC(y, mo, d + 1, 0, 0, 0, 0));
  
    strapi.log.debug(
      `listForDay invoked, UTC range ${dayStartUtc.toISOString()} -> ${dayEndUtc.toISOString()}`
    );
  
    const sessions = await strapi.entityService.findMany('api::event-session.event-session', {
      filters: {
        startDateTime: {
          $gte: dayStartUtc.toISOString(),
          $lt: dayEndUtc.toISOString(),
        },
      },
      fields: ['id', 'title', 'type', 'startDateTime', 'durationMinutes', 'maxCapacity'],
      populate: {
        product: {
          fields: ['id', 'name', 'slug', 'price', 'price_sale', 'inSale', 'locale', 'documentId'] as any,
          populate: {
            localizations: { fields: ['id', 'slug', 'locale'] },
          },
        } as any,
